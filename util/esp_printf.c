...
/* 2007-05-03 Serge Zloto: Added %f format and null  */
/*                         when null string pointer  */
/*                                                   */
/*---------------------------------------------------*/
...
/* as directed by the padding and positioning flags. */
/*                                                   */
static void outnum( const long numI, const long base)
{
	long num = numI;
	charptr cp;
	int negative;
      ...
      ch = *(++ctrl);
		...
		switch (tolower(ch)) {
			...
			case 'f':
			{
				...
				pad_character = '0';
				num1 = 6;
				const int rest = ((int) (1000000 * f) - r);
				outnum(rest, 10);
